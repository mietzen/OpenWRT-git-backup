...
echo "     - CRITICAL: opkg update && opkg install luci-compat"
echo "     - For HTTPS repos: opkg install git git-http ca-bundle wget"
echo "     - For SSH repos: opkg install git wget"
echo "     - Restart uhttpd: /etc/init.d/uhttpd restart"
echo "  2. Configure your repository"
echo "  3. Generate SSH key or configure HTTPS auth"
echo "  4. Enable automatic backups"
echo ""
echo "Note: When installed via opkg package, ALL dependencies are"
echo "      automatically installed. Manual installation requires"
echo "      manual dependency installation as shown above."
echo ""
